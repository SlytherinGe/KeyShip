...
class SSDDDataset(DOTADataset):
    ...
    def load_annotations(self, ann_file):
        ...
        for img_id in img_ids:
            ...
            xml_path = osp.join(self.img_prefix, 'annotations_r',
                                f'{img_id}.xml')
    ...
    def get_ann_info(self, idx):
        ...
        xml_path = osp.join(self.img_prefix, 'annotations_r', f'{img_id}.xml')
        tree = ET.parse(xml_path)
        root = tree.getroot()
        bboxes = []
        labels = []
        ...
        for obj in root.findall('object'):
            ...
            bboxpoly = [float(bnd_box.find('x1').text),float(bnd_box.find('y1').text),
                       float(bnd_box.find('x2').text),float(bnd_box.find('y2').text),
                       float(bnd_box.find('x3').text),float(bnd_box.find('y3').text),
                       float(bnd_box.find('x4').text),float(bnd_box.find('y4').text),
            ]

            bboxpts = np.array(bboxpoly).astype(np.float32)
            try:
                x, y, w, h, a = poly2obb_np(bboxpts, self.version)
            except:  # noqa: E722
                continue
            if difficult:
...
@ROTATED_DATASETS.register_module()
class SSDDDatasetOfficial(SSDDDataset):

    def __init__(self, min_size=None, **kwargs):
        super().__init__(min_size, **kwargs)
    ...
    def get_ann_info(self, idx):
        ...
        if len(bboxes)== 0:
            ...
            polygons = np.array(polygons)
        if len(bboxes_ignore)== 0:
            bboxes_ignore = np.zeros((1, 5))
            labels_ignore = np.zeros((0, ))
            polygons_ignore = np.zeros((1, 8))
        else:
            bboxes_ignore = np.array(bboxes_ignore)
            labels_ignore = np.array(labels_ignore)
            polygons_ignore = np.array(polygons_ignore)
        ann = dict(
            ...
            labels=labels.astype(np.int64),
            polygons = polygons.astype(np.float32))
            # bboxes_ignore=np.zeros((1, 5)).astype(np.float32),
            # labels_ignore=np.zeros((0, )).astype(np.int64),
            # polygons_ignore = np.zeros((1, 8)).astype(np.float32))
