...
class DeltaXYWHAOBBoxSoftCoder(BaseBBoxCoder):
...
def min_ori_delta(src, tgt, angle_range='le90'):

    assert angle_range in ['oc', 'le90', 'le135']

    if angle_range == 'le90':
        ori = tgt - src
        keep_flag = (torch.abs(ori) < (np.pi/2)).float()
        delta = keep_flag * ori + \
                (1 - keep_flag) * (-torch.sign(ori)*(np.pi - torch.abs(ori)))
    else:
        raise NotImplementedError
...
@mmcv.jit(coderize=True)
def min_ori_recover(src, delta, angle_range='le90'):
    ...
    if angle_range == 'le90':
        ...
        rev_flag = (abs_res > (np.pi / 2)).float()
        res =  rev_flag * (-res.sign() * (np.pi - abs_res)) + \
                (1-rev_flag) * res
    else:
        raise NotImplementedError
...
def bbox2delta(proposals,
    ...
    if edge_swap:
        ...
        # da_test = norm_angle(ga - pa, angle_range)
        da = min_ori_delta(pa, ga, angle_range)
        dw = torch.log(gw / pw)
        dh = torch.log(gh / ph)
...
def delta2bbox(rois,
    ...
    if proj_xy:
        ...
        gy = py + dy_height
    # Compute angle
    ga = min_ori_recover(pa, da, angle_range)
